...
function Modal({ isOpen, title, message, warning, onConfirm, onCancel, confirmText = 'Delete Forever', cancelText = 'Cancel', isLoading = false }) {
  return (
    <div 
      ...
      onClick={onCancel}
    >
      {/* [BUG - LAYERS] Relative positioning causes modal to break out of fixed container [FIX: change position-relative to remove it] */}
      <div 
        className={`relative bg-white rounded-2xl shadow-2xl max-w-sm w-full overflow-hidden transform transition-all ${
          isOpen ? 'scale-100 opacity-100' : 'scale-95 opacity-0'
        }`}
        onClick={(e) => e.stopPropagation()}
      >
        <div className="p-8 text-center">
          {/* [BUG - TYPO] Invalid Tailwind class bg-red-5 doesn't exist [FIX: change to bg-red-100] */}
          <div className="mx-auto flex items-center justify-center h-16 w-16 rounded-full bg-red-5 mb-6 animate-pulse">
